...
export async function updateUserService(uuid: string, { info, email, password, ...payload }: UpdateUserDTO) {
  ...
  const validatePassword = await recursiveFindUserPassword(
    user,
    [...userPasswords],
    password
  );
...
const recursiveFindUserPassword = async (
  user: UserEntity,
  passwordsToCompare: UserPasswordEntity[],
  password: string
): Promise<unknown> => {
  ...
  if (payload.password !== password)
    return recursiveFindUserPassword(user, passwordsToCompare, password);
  if (payload.password === password) return true;

  return false;
};

const recursiveRemoveAllUserPassword = async (
  user: UserEntity,
  passwordsToCompare: UserPasswordEntity[]
): Promise<unknown> => {
  const payload = await passwordsToCompare.pop();
